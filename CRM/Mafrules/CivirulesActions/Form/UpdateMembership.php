 ...
 */
class CRM_Mafrules_CivirulesActions_Form_UpdateMembership extends CRM_Core_Form {
  ...
  function buildQuickForm() {
    ...
    $membership_type = $this->addElement('advmultiselect', 'membership_type',
      ts('Available membership types') . ' ', $this->getMembershipTypes(),
      array('size' => 5,
        'style' => 'width:350px',
        'class' => 'advmultiselect')
    );
    $membership_type->setButtonAttributes('add', array('value' => ts('Include >>')));
    $membership_type->setButtonAttributes('remove', array('value' => ts('<< Exclude')));

    $membership_status = $this->addElement('advmultiselect', 'membership_status',
      ts('Available membership status') . ' ', $this->getMemershipStatus(),
      array('size' => 5,
        'style' => 'width:350px',
        'class' => 'advmultiselect')
    );
    $membership_status->setButtonAttributes('add', array('value' => ts('Include >>')));
    $membership_status->setButtonAttributes('remove', array('value' => ts('<< Exclude')));

    $this->addFormRule(array('CRM_Mafrules_CivirulesActions_Form_UpdateMembership', 'validateForm'));
